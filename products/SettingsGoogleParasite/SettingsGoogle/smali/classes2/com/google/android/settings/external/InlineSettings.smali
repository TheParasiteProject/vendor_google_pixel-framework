.class public abstract Lcom/google/android/settings/external/InlineSettings;
.super Ljava/lang/Object;
.source "InlineSettings.java"


# static fields
.field public static final AUTO_ROTATE_FACE_DETECTION_SETTING:Lcom/google/android/settings/external/specialcase/AutoRotateFaceDetectionSetting;

.field public static final BOLD_TEXT_SETTING:Lcom/google/android/settings/external/specialcase/BoldTextSetting;

.field public static final COLOR_INVERSION_SETTING:Lcom/google/android/settings/external/specialcase/ColorInversionSetting;

.field public static final DATA_SAVER_SETTING:Lcom/google/android/settings/external/specialcase/DataSaverSetting;

.field public static final DEVELOPER_OPTIONS_SETTING:Lcom/google/android/settings/external/specialcase/DeveloperOptionsSetting;

.field public static final EXTREME_BATTERY_SAVER_SETTING:Lcom/google/android/settings/external/specialcase/ExtremeBatterySaverSetting;

.field public static final HIGH_CONTRAST_MODE_SETTING:Lcom/google/android/settings/external/specialcase/HighContrastModeSetting;

.field public static final HOTSPOT_SETTING:Lcom/google/android/settings/external/specialcase/HotspotSetting;

.field public static final LOCATION_SETTING:Lcom/google/android/settings/external/specialcase/LocationSetting;

.field public static final MAGNIFY_GESTURE_SETTING:Lcom/google/android/settings/external/specialcase/MagnificationSetting;

.field public static final MAGNIFY_NAVBAR_SETTING:Lcom/google/android/settings/external/specialcase/MagnificationSetting;

.field public static final MOBILE_DATA_SETTING:Lcom/google/android/settings/external/specialcase/MobileDataSetting;

.field public static final NFC_SETTING:Lcom/google/android/settings/external/specialcase/NfcSetting;

.field public static final NIGHTDISPLAY_SETTING:Lcom/google/android/settings/external/specialcase/NightDisplaySetting;

.field public static final NOTIFICATION_BUBBLES_SETTING:Lcom/google/android/settings/external/specialcase/NotificationBubblesSetting;

.field public static final SCREEN_ATTENTION_SETTING:Lcom/google/android/settings/external/specialcase/ScreenAttentionSetting;

.field public static final SCREEN_TIMEOUT_SETTING:Lcom/google/android/settings/external/specialcase/ScreenTimeoutSetting;

.field public static final TALKBACK_SETTING:Lcom/google/android/settings/external/specialcase/AccessibilitySetting;

.field public static final USB_TETHERING_SETTING:Lcom/google/android/settings/external/specialcase/UsbTetheringSetting;

.field public static final VIBRATION_SETTING:Lcom/google/android/settings/external/specialcase/VibrationSetting;

.field public static final WIFI_SETTING:Lcom/google/android/settings/external/specialcase/WifiSetting;


# direct methods
.method static constructor <clinit>()V
    .locals 3

    .line 30
    new-instance v0, Lcom/google/android/settings/external/specialcase/AutoRotateFaceDetectionSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/AutoRotateFaceDetectionSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->AUTO_ROTATE_FACE_DETECTION_SETTING:Lcom/google/android/settings/external/specialcase/AutoRotateFaceDetectionSetting;

    .line 32
    new-instance v0, Lcom/google/android/settings/external/specialcase/BoldTextSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/BoldTextSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->BOLD_TEXT_SETTING:Lcom/google/android/settings/external/specialcase/BoldTextSetting;

    .line 33
    new-instance v0, Lcom/google/android/settings/external/specialcase/ColorInversionSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/ColorInversionSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->COLOR_INVERSION_SETTING:Lcom/google/android/settings/external/specialcase/ColorInversionSetting;

    .line 34
    new-instance v0, Lcom/google/android/settings/external/specialcase/DataSaverSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/DataSaverSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->DATA_SAVER_SETTING:Lcom/google/android/settings/external/specialcase/DataSaverSetting;

    .line 35
    new-instance v0, Lcom/google/android/settings/external/specialcase/DeveloperOptionsSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/DeveloperOptionsSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->DEVELOPER_OPTIONS_SETTING:Lcom/google/android/settings/external/specialcase/DeveloperOptionsSetting;

    .line 37
    new-instance v0, Lcom/google/android/settings/external/specialcase/ExtremeBatterySaverSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/ExtremeBatterySaverSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->EXTREME_BATTERY_SAVER_SETTING:Lcom/google/android/settings/external/specialcase/ExtremeBatterySaverSetting;

    .line 39
    new-instance v0, Lcom/google/android/settings/external/specialcase/HighContrastModeSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/HighContrastModeSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->HIGH_CONTRAST_MODE_SETTING:Lcom/google/android/settings/external/specialcase/HighContrastModeSetting;

    .line 41
    new-instance v0, Lcom/google/android/settings/external/specialcase/HotspotSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/HotspotSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->HOTSPOT_SETTING:Lcom/google/android/settings/external/specialcase/HotspotSetting;

    .line 42
    new-instance v0, Lcom/google/android/settings/external/specialcase/LocationSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/LocationSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->LOCATION_SETTING:Lcom/google/android/settings/external/specialcase/LocationSetting;

    .line 44
    new-instance v0, Lcom/google/android/settings/external/specialcase/MagnificationSetting;

    const-string v1, "magnify_gesture"

    const-string v2, "accessibility_display_magnification_enabled"

    invoke-direct {v0, v1, v2}, Lcom/google/android/settings/external/specialcase/MagnificationSetting;-><init>(Ljava/lang/String;Ljava/lang/String;)V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->MAGNIFY_GESTURE_SETTING:Lcom/google/android/settings/external/specialcase/MagnificationSetting;

    .line 47
    new-instance v0, Lcom/google/android/settings/external/specialcase/MagnificationSetting;

    const-string v1, "magnify_navbar"

    const-string v2, "accessibility_display_magnification_navbar_enabled"

    invoke-direct {v0, v1, v2}, Lcom/google/android/settings/external/specialcase/MagnificationSetting;-><init>(Ljava/lang/String;Ljava/lang/String;)V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->MAGNIFY_NAVBAR_SETTING:Lcom/google/android/settings/external/specialcase/MagnificationSetting;

    .line 50
    new-instance v0, Lcom/google/android/settings/external/specialcase/MobileDataSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/MobileDataSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->MOBILE_DATA_SETTING:Lcom/google/android/settings/external/specialcase/MobileDataSetting;

    .line 51
    new-instance v0, Lcom/google/android/settings/external/specialcase/NfcSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/NfcSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->NFC_SETTING:Lcom/google/android/settings/external/specialcase/NfcSetting;

    .line 52
    new-instance v0, Lcom/google/android/settings/external/specialcase/NightDisplaySetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/NightDisplaySetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->NIGHTDISPLAY_SETTING:Lcom/google/android/settings/external/specialcase/NightDisplaySetting;

    .line 53
    new-instance v0, Lcom/google/android/settings/external/specialcase/NotificationBubblesSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/NotificationBubblesSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->NOTIFICATION_BUBBLES_SETTING:Lcom/google/android/settings/external/specialcase/NotificationBubblesSetting;

    .line 55
    new-instance v0, Lcom/google/android/settings/external/specialcase/ScreenAttentionSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/ScreenAttentionSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->SCREEN_ATTENTION_SETTING:Lcom/google/android/settings/external/specialcase/ScreenAttentionSetting;

    .line 57
    new-instance v0, Lcom/google/android/settings/external/specialcase/ScreenTimeoutSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/ScreenTimeoutSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->SCREEN_TIMEOUT_SETTING:Lcom/google/android/settings/external/specialcase/ScreenTimeoutSetting;

    .line 58
    new-instance v0, Lcom/google/android/settings/external/specialcase/AccessibilitySetting;

    const-string v1, "talkback"

    const-string v2, "com.google.android.marvin.talkback/com.google.android.marvin.talkback.TalkBackService"

    invoke-direct {v0, v1, v2}, Lcom/google/android/settings/external/specialcase/AccessibilitySetting;-><init>(Ljava/lang/String;Ljava/lang/String;)V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->TALKBACK_SETTING:Lcom/google/android/settings/external/specialcase/AccessibilitySetting;

    .line 62
    new-instance v0, Lcom/google/android/settings/external/specialcase/UsbTetheringSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/UsbTetheringSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->USB_TETHERING_SETTING:Lcom/google/android/settings/external/specialcase/UsbTetheringSetting;

    .line 63
    new-instance v0, Lcom/google/android/settings/external/specialcase/VibrationSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/VibrationSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->VIBRATION_SETTING:Lcom/google/android/settings/external/specialcase/VibrationSetting;

    .line 64
    new-instance v0, Lcom/google/android/settings/external/specialcase/WifiSetting;

    invoke-direct {v0}, Lcom/google/android/settings/external/specialcase/WifiSetting;-><init>()V

    sput-object v0, Lcom/google/android/settings/external/InlineSettings;->WIFI_SETTING:Lcom/google/android/settings/external/specialcase/WifiSetting;

    return-void
.end method
